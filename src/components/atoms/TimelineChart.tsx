...
export default function TimelineChart({
    ...
    const firstReleasePosition =
        ((new Date(data.firstReleaseDate).getTime() - minEpoch) / maxDuration) *
        100;
        ...
        <div className="h-8 w-full relative">
            ...
            {data.minorVersions.map((minor, i) => {
                const releasePosition =
                    ((new Date(minor.releaseDate).getTime() - minEpoch) /
                        maxDuration) *
                    100;
                const nextRelease =
                ...
                const width = nextRelease
                    ? ((new Date(nextRelease).getTime() -
                          new Date(minor.releaseDate).getTime()) /
                          maxDuration) *
                      100
                    : 2;
