...
program
    ...
    .description(
        "Update relative links to absolute links in the formatted JSON",
    )
...
async function parseReleaseNotes(directory: string) {
    ...
    for (const file of files) {
        if (path.extname(file) === ".html" && file !== "index.html") {
            ...
            let releaseDateElement = $("p:contains('Release date:')");
            if (releaseDateElement.length === 0) {
                releaseDateElement = $("b:contains('Release Date:')").parent();
            }
            const releaseDate = releaseDateElement.text().split(":")[1].trim();

            const changes: string[] = [];

            $(".itemizedlist ul li.listitem").each((index, element) => {
                const processedHtml = processText(
                    $(element).html().trim(),
                    baseUrl,
                );
...
function categorizeChanges(changes: string[], is_major: boolean = false) {
    ...
    changes.forEach((change) => {
        ...
        for (const [category, words] of Object.entries(keywords)) {
            if (
                !is_major &&
                (category == "features" || category == "performance")
            ) {
                break;
            }
            ...
            if (!("metrics" in security)) {
                ...
                if (
                    ...
                    security.severity = security.metrics.cvssMetricV30
                        ? security.metrics.cvssMetricV30[0].cvssData
                              .baseSeverity
                        : security.metrics.cvssMetricV31
                          ? security.metrics.cvssMetricV31[0].cvssData
                                .baseSeverity
...
function extractTitleDescription(item: string): {
    title: string;
    description: string;
} {
            ...
            if (json[category] && Array.isArray(json[category])) {
                json[category] = json[category].map((item) => {
                    if (typeof item === "object") {
                        const version =
                            item.fixedIn || item.sinceVersion || "0";
                        return {
                            ...item,
                            title: item.title
                                ? killLinks(item.title)
                                : item.title,
                            description: item.description
                                ? updateLinks(item.description, version)
                                : item.description,
                        };
                    }
                    return item;
